...
database and its audit invariants exercised. It applies cleanly. Four defects were
found, one of them critical.
...
## 2. `identity.devices` has no WebAuthn sign counter
...
## 3. `identity.sessions.step_up_verified` never expires
...
## 4. `break_glass_credentials.status` is a value list, not a state machine
