...
def _execute(*statements: str) -> None:
    """Execute separately because asyncpg rejects multi-command prepares."""
    ...
    _execute(
        ...
        BEGIN
          IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='finance'
                     AND table_name='ledger_account_mappings'
                     AND column_name='tally_ledger_name') THEN
            ALTER TABLE finance.ledger_account_mappings
              RENAME COLUMN tally_ledger_name TO external_account_name;
          END IF;
          IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='finance'
                     AND table_name='ledger_account_mappings'
                     AND column_name='erp_reference_type') THEN
            ALTER TABLE finance.ledger_account_mappings
              RENAME COLUMN erp_reference_type TO atlas_reference_type;
            ALTER TABLE finance.ledger_account_mappings
              RENAME COLUMN erp_reference_id TO atlas_reference_id;
          END IF;
          IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='finance'
                     AND table_name='external_vouchers'
                     AND column_name='import_batch_id') THEN
            ALTER TABLE finance.external_vouchers
              RENAME COLUMN import_batch_id TO sync_batch_id;
          END IF;
          IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='finance'
            ...
            ALTER TABLE finance.reconciliations
              RENAME COLUMN erp_reference_id TO atlas_reference_id;
            ALTER TABLE finance.reconciliations
              RENAME COLUMN tally_voucher_id TO external_voucher_id;
        ...
        BEGIN
          ...
          IF constraint_name IS NOT NULL THEN
            EXECUTE format(
              'ALTER TABLE finance.reconciliations DROP CONSTRAINT %I', constraint_name
            );
          END IF;
        END $$
        """,
        """UPDATE finance.reconciliations
        SET discrepancy_type='missing_in_external_ledger'
        WHERE discrepancy_type='missing_in_tally'""",
        """UPDATE finance.reconciliations
        SET discrepancy_type='missing_in_atlas'
        WHERE discrepancy_type='missing_in_erp'""",
        """ALTER TABLE finance.ledger_sync_batches
        DROP CONSTRAINT IF EXISTS ledger_sync_batches_provider_check""",
        """ALTER TABLE finance.ledger_sync_batches
        ADD CONSTRAINT ledger_sync_batches_provider_check
        CHECK (provider ~ '^[a-z][a-z0-9_]{1,31}$')""",
        """ALTER TABLE finance.ledger_account_mappings
        DROP CONSTRAINT IF EXISTS ledger_account_mappings_provider_check""",
        """ALTER TABLE finance.ledger_account_mappings
        ADD CONSTRAINT ledger_account_mappings_provider_check
        CHECK (provider ~ '^[a-z][a-z0-9_]{1,31}$')""",
        """ALTER TABLE finance.reconciliations
        ADD CONSTRAINT reconciliations_discrepancy_type_check
        CHECK (discrepancy_type IN (
          'missing_in_external_ledger','missing_in_atlas','amount_mismatch',
